...
func callHost(method string, payload []byte) ([]byte, bool) {
	...
	if C.call_host_api(cMethod, reqPtr, C.size_t(len(payload)), &response) != 0 || response.ptr == nil {
		return nil, false
	}
...
// hostLog 把插件日志交给宿主的日志系统。
func hostLog(level, message string) {
	raw, errMarshal := json.Marshal(map[string]any{"level": level, "message": message})
	if errMarshal != nil {
		return
	}
